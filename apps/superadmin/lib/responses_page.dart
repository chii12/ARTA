...
class _ResponsesPageState extends State<ResponsesPage> {
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ElevatedButton.icon(
                            onPressed: () => _exportResponse(respondent, service, date, answersMap, suggestions),
                            icon: const Icon(Icons.download, size: 18),
    ...
    return Table(
      ...
      children: [
        ...
        ...questions.asMap().entries.map((e) {
          ...
          debugPrint('DEBUG: $code answer = $answer (${answer.runtimeType})');
          return TableRow(
            children: [
  ...
  void _exportResponse(Map<String, dynamic>? respondent, Map<String, dynamic>? service, DateTime date, Map<String, dynamic> answersMap, Map<String, dynamic>? suggestions) {
    final exportData = _generateExportData(respondent, service, date, answersMap, suggestions);
    
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Response data exported: ${exportData.length} characters'),
  ...
  String _generateExportData(Map<String, dynamic>? respondent, Map<String, dynamic>? service, DateTime date, Map<String, dynamic> answersMap, Map<String, dynamic>? suggestions) {
    final buffer = StringBuffer();
    
    buffer.writeln('ARTA SURVEY RESPONSE EXPORT');
    buffer.writeln('=' * 40);
    buffer.writeln('Date: ${date.month}/${date.day}/${date.year}');
    buffer.writeln('Name: ${respondent?['name'] ?? 'N/A'}');
    buffer.writeln('Client Type: ${respondent?['client_type'] ?? 'N/A'}');
    buffer.writeln('Sex: ${respondent?['sex'] ?? 'N/A'}');
    buffer.writeln('Age: ${respondent?['age'] ?? 'N/A'}');
    buffer.writeln('Region: ${respondent?['region_of_residence'] ?? 'N/A'}');
    buffer.writeln('Service: ${service?['service_name'] ?? 'N/A'}');
    buffer.writeln();
    ...
    if (suggestions != null) {
      ...
      buffer.writeln(suggestions['comment_text'] ?? 'None');
      buffer.writeln('Email: ${suggestions['email_optional'] ?? 'None'}');
    }
